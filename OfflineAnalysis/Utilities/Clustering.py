...
def clustering_DPA_multiprocessing(m,rand_idx, queue, dpa_z=OfflineConfig.dpa_z,k_max=OfflineConfig.dpa_k_max):
    """
    Density peak clustering. Find density peaks in low dimensional space, tweak Z
    :param m:
    :param rand_idx:
...
def merging_spurious_labels_multiprocessing(m,rand_idx,label_dict,est, queue):
    #change the dictionary format
    label_dict = {vi: k for k, v in label_dict.items() for vi in v}
    #merge the labels
    est.labels_ = np.vectorize(label_dict.get)(est.labels_)
